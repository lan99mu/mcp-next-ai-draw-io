...
基于 Python 的 Model Context Protocol (MCP) 服务器，用于生成和编辑 Draw.io 图表文件，并提供**实时浏览器预览**。

## 🎯 主要改进

相比基础版本，此增强版本参考了 [next-ai-draw-io](https://github.com/DayuanJiang/next-ai-draw-io) 项目，新增以下重要功能：

- 🌐 **实时浏览器预览** - 内置 HTTP 服务器，在浏览器中实时显示图表更新
- ✏️ **图表编辑功能** - 支持基于 ID 的更新、添加、删除操作
- 📜 **版本历史** - 自动保存图表历史版本，可查看和恢复
- 💾 **导出功能** - 直接导出为 .drawio 文件
- 🔄 **状态同步** - AI 与浏览器状态自动同步

相比只能生成 XML 的基础版本，增强版提供完整的工作流：
1. **启动会话** → 打开浏览器进行实时预览
2. **创建/编辑图表** → 在浏览器中即时查看更改
3. **导出** → 保存为 .drawio 文件

## 特性

- ✍️ **生成 Draw.io XML 文件** - 以编程方式创建 .drawio 图表文件
- 🌐 **实时浏览器预览** - 在浏览器中即时查看图表更新
- 🔷 **多种形状类型** - 支持矩形、椭圆、菱形等多种形状
- ✏️ **编辑现有图表** - 通过 ID 更新、添加或删除图表元素
- 🔗 **连接形状** - 在形状之间添加带自定义箭头的连接线
- 📜 **版本历史** - 跟踪和恢复之前的图表版本
- 💾 **导出文件** - 将图表保存为 .drawio 文件
- 💾 **标准格式** - 输出与 Draw.io 和 diagrams.net 兼容
- 🤖 **MCP 兼容** - 可与 VS Code Copilot、Claude Desktop 等 MCP 客户端配合使用
